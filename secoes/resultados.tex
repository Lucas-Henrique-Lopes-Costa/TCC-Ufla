...
Neste capítulo são descritas as principais funcionalidades desenvolvidas durante o estágio para a plataforma de protocolos da \novo{empresa cliente}. A apresentação está organizada por módulo, detalhando as decisões técnicas tomadas e os desafios enfrentados em cada etapa. A seleção das atividades se deu por meio da sua relevância no escopo do projeto e pela complexidade técnica envolvida.

\section{Módulo de Autenticação e Controle de Acesso}

O primeiro módulo desenvolvido foi o de autenticação, responsável por controlar o acesso dos colaboradores da \novo{empresa cliente} à plataforma. A tela de login foi construída seguindo a identidade visual da empresa cliente, com campos para e-mail e senha e a opção de recuperação de senha.

A autenticação foi implementada com o uso da biblioteca \textit{jwt-decode} para decodificação de \novo{\textit{tokens}} JWT (\textit{JSON Web Token}) recebidos da API. Após o login bem-sucedido, os dados da sessão (\textit{access token} e \textit{refresh token}) e as informações do usuário extraídas do \textit{token} eram armazenados no estado global da aplicação, gerenciado pela biblioteca \textit{Zustand} com o \textit{middleware} de persistência. A atualização imutável desse estado foi feita com o auxílio da biblioteca \textit{Immer}. O \autoref{cod:store} apresenta um trecho simplificado da função que processa o login, decodificando o \textit{token} e populando o \textit{store}.
...
\novop{A \autoref{fig:tela-login} apresenta a tela de \textit{login} da plataforma, que era o ponto de entrada de todos os usuários.}

\begin{figure}[!htb]
\centering
\caption{\novo{Tela de \textit{login} da plataforma.}}
\label{fig:tela-login}
\includegraphics[width=0.75\textwidth,height=0.35\textheight,keepaspectratio]{imgs/tela-login.png}
\fonte{\novo{do autor (2026).}}
\end{figure}

\novop{Já a gestão de usuários, acessível apenas pelo perfil Administrador, permitia visualizar, adicionar, editar e excluir os colaboradores cadastrados. A listagem exibia nome, tipo de perfil, e-mail e contato de cada usuário, com suporte a busca textual e filtros por tipo de perfil, conforme apresentado na \autoref{fig:tela-usuarios}. As operações de criação e exclusão de usuários eram realizadas por meio de modais, componentes sobrepostos à tela principal que permitem ao usuário executar uma ação sem perder o contexto da página atual. Foram implementados modais para o formulário de adição de usuário, para a confirmação de exclusão e para o \textit{feedback} de sucesso e erro, ilustrados na \autoref{fig:tela-modais} junto com as variações do painel de filtros. Todos os formulários utilizavam a biblioteca \textit{Formik} para gerenciamento de estado dos campos e \textit{Yup} para validação dos dados antes do envio à API.}

\begin{figure}[!htb]
\centering
\caption{\novo{Tela de gestão de usuários.}}
\label{fig:tela-usuarios}
\includegraphics[width=0.95\textwidth,height=0.40\textheight,keepaspectratio]{imgs/tela-gestao-usuarios.png}
\fonte{\novo{do autor (2026).}}
\end{figure}

\begin{figure}[!htb]
\centering
\caption{\novo{Modais de filtro, adição, exclusão e \textit{feedback} da gestão de usuários.}}
\label{fig:tela-modais}
\includegraphics[width=0.95\textwidth,height=0.42\textheight,keepaspectratio]{imgs/tela-modais.png}
\fonte{\novo{do autor (2026).}}
\end{figure}
...
\begin{figure}[!htb]
\centering
\caption{\novo{Aba de Avaliações do formulário de protocolo.}}
\label{fig:tela-avaliacoes}
\includegraphics[width=0.95\textwidth,height=0.50\textheight,keepaspectratio]{imgs/tela-protocolo-avaliacoes.png}
\fonte{\novo{do autor (2026).}}
\end{figure}
...
\novop{O mesmo padrão de organização de formulários (estrutura em abas, agrupamento de campos por área e separação visual entre seções obrigatórias e opcionais) foi reutilizado em outros módulos da plataforma, como o de orçamentos. A \autoref{fig:tela-orcamento} apresenta variações da tela de Informações desse módulo, incluindo o estado padrão e estados com mensagens de validação, além de uma amostra dos componentes de seleção (\textit{selects}) reaproveitados em diferentes contextos.}

\begin{figure}[!htb]
\centering
\caption{\novo{Tela de Informações do módulo de orçamentos e componentes reutilizados.}}
\label{fig:tela-orcamento}
\includegraphics[width=0.95\textwidth,height=0.55\textheight,keepaspectratio]{imgs/tela-orcamento-formulario.png}
\fonte{\novo{do autor (2026).}}
\end{figure}

Uma das principais dificuldades encontradas neste módulo foi a tradução das regras de negócio da \novo{empresa cliente} para a lógica do formulário. As regras variavam conforme o tipo de protocolo, a cultura selecionada e a classe do experimento, exigindo uma comunicação constante com a equipe do cliente para validar o comportamento esperado. O uso da biblioteca \textit{Yup} para a definição de esquemas de validação mostrou-se essencial para garantir a integridade dos dados antes do envio ao servidor, distinguindo os campos obrigatórios dos opcionais e validando tipos, listas e formatos. O \autoref{cod:yup} apresenta um trecho do esquema de validação da aba de Informações do protocolo.
...
O módulo de análise e \novo{\textit{dashboards}} representou o principal diferencial da plataforma em relação ao uso de planilhas. A proposta era transformar os dados brutos registrados nos protocolos em visualizações gráficas que auxiliassem os pesquisadores na tomada de decisões.
...
\novop{A \autoref{fig:tela-graficos} apresenta o resultado da geração automatizada dos gráficos de barras a partir dos dados de uma avaliação. Cada gráfico ocupa um \textit{card} próprio, dispostos em uma grade na tela, com as colunas coloridas por categoria e os valores numéricos exibidos sobre elas, de modo que o pesquisador consegue comparar os momentos de avaliação sem precisar montar manualmente as visualizações. Além do \textit{download} individual disponível em cada \textit{card}, a tela oferecia a opção de baixar todos os gráficos de uma só vez.}

\begin{figure}[!htb]
\centering
\caption{\novo{Gráficos de barras gerados automaticamente para uma avaliação.}}
\label{fig:tela-graficos}
\includegraphics[width=0.72\textwidth,height=0.60\textheight,keepaspectratio]{imgs/tela-graficos-avaliacao.png}
\fonte{\novo{do autor (2026).}}
\end{figure}

\novop{Já a \autoref{fig:tela-exportacao} ilustra o produto final do processo de exportação descrito no \autoref{cod:export}. A imagem capturada reúne, em um único arquivo PNG, o gráfico selecionado, o logotipo da empresa cliente e o título dos dados representados, resultando em um material pronto para ser incluído nos relatórios entregues pela empresa aos seus próprios clientes.}

\begin{figure}[!htb]
\centering
\caption{\novo{Imagem PNG exportada de um gráfico, com a identidade visual da empresa.}}
\label{fig:tela-exportacao}
\includegraphics[width=0.65\textwidth,height=0.48\textheight,keepaspectratio]{imgs/tela-graficos-exportacao.png}
\fonte{\novo{do autor (2026).}}
\end{figure}

\subsection{Dashboard Geral}

Além dos gráficos individuais por protocolo, a plataforma contava com um \novo{\textit{dashboard}} geral que apresentava uma visão consolidada de todos os protocolos cadastrados, incluindo a distribuição de protocolos por classe, por cultura e por status. \novop{A \autoref{fig:tela-dashboard} apresenta essa tela, em que os gráficos de pizza resumem a divisão dos protocolos por tipo e por status e os gráficos de barras horizontais detalham a distribuição por classe, por cultura e por cliente.}

\begin{figure}[!htb]
\centering
\caption{\novo{\textit{Dashboard} geral com a visão consolidada dos protocolos.}}
\label{fig:tela-dashboard}
\includegraphics[width=0.60\textwidth,height=0.86\textheight,keepaspectratio]{imgs/tela-dashboard-geral.png}
\fonte{\novo{do autor (2026).}}
\end{figure}
...
Uma das dificuldades técnicas enfrentadas na construção dos \novo{\textit{dashboards}} foi a performance de renderização quando o volume de dados era elevado. A renderização simultânea de múltiplos gráficos com grandes conjuntos de dados causava lentidão perceptível na interface. Para mitigar esse problema, foi adotada uma estratégia de carregamento sob demanda (\textit{lazy loading}), em que os gráficos eram renderizados apenas quando o usuário navegava até a seção correspondente.
